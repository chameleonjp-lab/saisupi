...
- 目標マスを円形の囲いではなく、余白を持たせた角丸のマス・縁取り・読みやすい賽の目で描くようにした。
- 一致した目標マスの四辺に沿う4本の半透明な光の柱を上方向へ伸ばし、1〜9個目では3,000msで成長・フェードして破棄する。10個目は光柱・待ち時間なしで、スコア確定直後に結果画面へ表示する。
...
- 目標マスに余白のある角丸土台、四角い縁取り、読みやすい賽の目を追加し、盤面に質感・縁取り・フレームを加えた。サイコロは角の丸み・輪郭・材質、キャラクターは目・瞳・口・材質を改善した。7×7、固定座標、6面対応、キャラクター倍率`0.9`は維持する。
